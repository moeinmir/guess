...
export const BetsProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const fetchBets = async () => {
    ...
    try {
      ...
      const bets: Bet[] = [];
      let startIndex =
        Number(lastBetId) - (reversePageNumber + 1) * pageSize < 0
          ? 0
          : Number(lastBetId) - (reversePageNumber + 1) * pageSize;
      let endIndex =
        Number(lastBetId) - reversePageNumber * pageSize > lastBetId
          ? lastBetId
          : Number(lastBetId) - reversePageNumber * pageSize;
      ...
      const winningBets = bets.filter(
        (bet) =>
          bet.isClosed && bet.closestGuessAddress == account && !bet.isSettled
      );
      const settledWinningBets = bets.filter(
        (bet) => bet.isSettled && bet.closestGuessAddress == account
      );
      setState({
        activeBets,
        closedBets,
